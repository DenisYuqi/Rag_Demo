...
应用会读取仓库根目录的 `.env` 和 `.env.local`，其中后加载的 `.env.local` 可覆盖 `.env`；进程环境变量的优先级高于 dotenv 文件。配置在进程启动时加载并缓存，修改后必须重启应用。

建议用途：

- `.env.example`：可提交的安全模板，不包含真实凭据。
- `.env`：本机非敏感基线，可从示例文件复制。
- `.env.local`：本机密钥和个人覆盖，不提交 Git。
- 生产环境：平台环境变量或 Secret 文件，不依赖工作目录中的 dotenv。

## 最小可用配置

源代码方式启动时，可在 `.env.local` 中配置：
