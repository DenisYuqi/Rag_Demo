...
def header(page: fitz.Page, section: str, page_no: int) -> None:
    ...
    textbox(
        page, fitz.Rect(MARGIN, 13, 300, 42), "安宁人寿 · 演示保单", size=14, color=WHITE, bold=True
    )
    textbox(
        page,
        fitz.Rect(300, 16, PAGE.width - MARGIN, 40),
        section,
        size=9,
        color=WHITE,
    ...
    textbox(
        page,
        fitz.Rect(MARGIN, PAGE.height - 29, PAGE.width - MARGIN, PAGE.height - 12),
        f"示例文件｜非有效保险合同                              第 {page_no} 页 / 共 5 页",
        size=8,
        color=MUTED,
...
def title(page: fitz.Page, y: float, number: str, name: str) -> float:
    page.draw_rect(
        fitz.Rect(MARGIN, y, PAGE.width - MARGIN, y + 31), color=BLUE, fill=PALE, width=0.8
    )
    textbox(
        page,
        fitz.Rect(MARGIN + 12, y + 6, PAGE.width - MARGIN - 8, y + 27),
        f"{number}  {name}",
        size=12,
        color=NAVY,
        bold=True,
...
def table(page: fitz.Page, y: float, rows: list[tuple[str, str]], *, row_h: float = 31) -> float:
    ...
    for idx, (label, value) in enumerate(rows):
        ...
        page.draw_rect(
            fitz.Rect(left, top, right, top + row_h), color=(0.78, 0.83, 0.87), fill=fill, width=0.5
        )
        page.draw_line(
            fitz.Point(split, top),
            fitz.Point(split, top + row_h),
            color=(0.78, 0.83, 0.87),
            width=0.5,
        )
        textbox(
            page,
            fitz.Rect(left + 9, top + 7, split - 6, top + row_h - 4),
            label,
            size=9,
            color=NAVY,
            bold=True,
...
def bullets(page: fitz.Page, y: float, items: list[str], *, gap: float = 45) -> float:
    for item in items:
        ...
        textbox(
            page,
            fitz.Rect(MARGIN + 16, y, PAGE.width - MARGIN, y + gap - 3),
            item,
            size=9.3,
            lineheight=1.45,
        )
...
def cover(doc: fitz.Document) -> None:
    ...
    textbox(
        page,
        fitz.Rect(MARGIN, 55, PAGE.width - MARGIN, 92),
        "安宁人寿保险股份有限公司",
        size=15,
        color=WHITE,
        bold=True,
    ...
    textbox(
        page,
        fitz.Rect(MARGIN, 111, PAGE.width - MARGIN, 160),
        "恒爱一生定期寿险",
        size=28,
        color=WHITE,
        bold=True,
    ...
    textbox(
        page,
        fitz.Rect(MARGIN, 164, PAGE.width - MARGIN, 198),
        "保险单（演示样本）",
        size=18,
    ...
    page.draw_rect(
        fitz.Rect(MARGIN, 286, PAGE.width - MARGIN, 421),
        color=(0.78, 0.83, 0.87),
        fill=PALE,
        width=0.8,
    )
    textbox(
        page,
        fitz.Rect(MARGIN + 22, 307, PAGE.width - MARGIN - 22, 338),
    ...
    textbox(
        page,
        fitz.Rect(MARGIN + 22, 349, PAGE.width - MARGIN - 22, 379),
        "保险期间：2026年8月8日零时至2046年8月7日二十四时",
        size=10,
    )
    textbox(
        page,
        fitz.Rect(MARGIN + 22, 386, PAGE.width - MARGIN - 22, 413),
        "基本保险金额：人民币 1,000,000.00 元",
        size=10,
        bold=True,
    )

    page.draw_rect(
        fitz.Rect(MARGIN, 467, PAGE.width - MARGIN, 560),
        color=RED,
        fill=(1.0, 0.95, 0.95),
        width=1.1,
    )
    textbox(
        page,
        fitz.Rect(MARGIN + 18, 486, PAGE.width - MARGIN - 18, 518),
        "重要声明",
        size=14,
        color=RED,
        bold=True,
    ...
    textbox(
        page,
        fitz.Rect(MARGIN + 22, 522, PAGE.width - MARGIN - 22, 552),
        "本文件仅用于软件演示、检索和测试，不代表任何真实承保关系，不具有法律效力。",
        size=10,
        color=RED,
        bold=True,
        align=fitz.TEXT_ALIGN_CENTER,
    ...
    textbox(
        page,
        fitz.Rect(MARGIN, 620, PAGE.width - MARGIN, 660),
        "签发日期：2026年8月7日",
        size=10,
        align=fitz.TEXT_ALIGN_CENTER,
    )
    textbox(
        page,
        fitz.Rect(MARGIN, 680, PAGE.width - MARGIN, 735),
        "客服热线：400-000-0000（虚构）\n公司地址：中国上海市示范路88号（虚构）",
        size=9,
        color=MUTED,
        align=fitz.TEXT_ALIGN_CENTER,
    )
...
def terms(doc: fitz.Document) -> None:
    ...
    page.draw_rect(
        fitz.Rect(MARGIN, y, PAGE.width - MARGIN, y + 150),
        color=(0.78, 0.83, 0.87),
        fill=(0.98, 0.99, 1),
        width=0.7,
    )
    textbox(
        page,
        fitz.Rect(MARGIN + 18, y + 17, PAGE.width - MARGIN - 18, y + 56),
        "投保人确认已阅读并理解保险责任、责任免除、犹豫期、退保损失等重要内容。",
        size=9.5,
    )
    textbox(
        page, fitz.Rect(MARGIN + 18, y + 76, 275, y + 108), "投保人签名：________________", size=9.5
    )
    textbox(
        page,
        fitz.Rect(310, y + 76, PAGE.width - MARGIN - 18, y + 108),
        "被保险人签名：________________",
        size=9.5,
    )
    textbox(
        page,
        fitz.Rect(MARGIN + 18, y + 116, PAGE.width - MARGIN - 18, y + 143),
        "签署日期：2026年8月7日",
        size=9.5,
    )
    textbox(
        page,
        fitz.Rect(MARGIN, y + 180, PAGE.width - MARGIN, y + 230),
        "安宁人寿保险股份有限公司（虚构）\n电子签章：仅供演示",
        size=10,
        color=NAVY,
        bold=True,
