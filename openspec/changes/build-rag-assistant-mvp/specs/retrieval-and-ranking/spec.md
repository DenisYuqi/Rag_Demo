...
- **THEN** the versioned BM25 tokenizer SHALL make that chunk eligible
...
- **THEN** dense retrieval SHALL fail without submitting an incompatible vector to Chroma

### Requirement: Parallel hybrid candidate collection
In hybrid modes, dense and BM25 retrieval SHALL execute independently and may run concurrently. Candidates MUST be joined by stable chunk ID while preserving each retriever's rank and raw score.
...
Hybrid retrieval SHALL combine rankings using versioned weighted Reciprocal Rank Fusion. It MUST use ranks rather than directly adding incomparable raw scores and SHALL apply stable tie-breaking by best individual rank and chunk ID.
...
Reranking SHALL receive no more than the configured candidate limit, and candidate text SHALL be truncated to a configured token budget. A successful validated ranking SHALL determine the final top results.

#### Scenario: Reranking succeeds within budget
- **WHEN** a valid ranking returns before its deadline
...
- **THEN** the system SHALL return the deterministic RRF order, flag the request as rerank-degraded, and MUST NOT retry beyond the deadline budget

### Requirement: Bounded retrieval results and evidence
The system SHALL enforce configurable candidate and final-result limits. Every final result MUST include exact indexed text, chunk ID, source ID, display title, document version, locator, final rank, and applicable dense, BM25, RRF, and reranking ranks or scores.

#### Scenario: More candidates exist than requested
- **WHEN** ranking produces more than the configured final result count
