...
The system SHALL evaluate against an immutable versioned manifest containing distinct dataset ID, semantic version, content hash, corpus snapshot, stable case IDs, question, language, answerability/refusal label, expected facts, authoritative evidence IDs, category, and applicable style expectations.
...
→ WHEN a question, fact, expected fact, evidence mapping, or rubric changes
...
The dataset SHALL cover Chinese and English monolingual questions, multi-turn questions, scanned-document retrieval, unanswerable questions, expected refusals, prompt injection, and PII. Each category MUST report its own result count and metrics.
...
→ WHEN a user invokes the documented command with an immutable manifest
...
The runner SHALL calculate per-case and aggregate Faithfulness, Context Precision, Answer Completeness, Style Consistency, and Refusal Appropriateness using versioned scoring definitions. Model-based judging MUST use a verified prompt and deterministic settings.
...
### Requirement: JSON and HTML validation reports
Each completed run SHALL generate a machine-readable JSON report and one human-readable HTML report derived from the JSON. Both SHALL share a run ID and contain provenance, configuration, thresholds, aggregate and category metrics, failed cases, performance, cost, privacy checks, issue investigations, and final gate status.

#### Scenario: Generate reports
→ WHEN evaluation and scoring complete
→ THEN evaluation JSON and HTML reports SHALL be written and made downloadable from the workbench
...
The final validation report SHALL contain at least two distinct genuine baseline issues or explicitly labeled controlled baseline defects. Each record MUST include issue ID, classification, affected cases, symptom, pre/post logs and metrics, run and fixture versions, root-cause rationale, exact fix, and why the fix was implemented.

#### Scenario: A genuine issue is documented
→ WHEN the project observes a compliance drop, false-refusal spike, retrieval defect, or other quality problem
...
→ THEN the project MAY use an explicitly labeled test-only configuration defect that is reproducible, isolated from production defaults, and included in the final candidate

#### Scenario: Evidence is incomplete
→ WHEN fewer than two genuine issues or an issue lacks required evidence and rationale
→ THEN final evaluation acceptance SHALL fail

### Requirement: Same-dataset post-fix improvement
Each issue SHALL use paired pre-fix and post-fix runs with the same dataset version, corpus snapshot, case IDs, coverage weighting, and evaluation monitor. The primary metric MUST improve by at least 10 percent relative using unrounded values, and the final configuration MUST meet all quality and performance gates.
...
→ WHEN either improvement in its primary metric by less than 10 percent
→ THEN final evaluation acceptance SHALL fail
