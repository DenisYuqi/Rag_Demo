...
Application logs and traces SHALL use allowed structured fields and MUST NOT record raw questions, answers, prompts, conversation histories, retrieved text, uploaded content, authorization headers, or credentials. Any permitted free-text error or metadata field MUST pass redaction before export.
...
→ THEN the system SHALL treat those queries as untrusted content and SHALL NOT execute or follow those instructions

#### Scenario: A legitimate question quotes injection language
→ WHEN a user asks a grounded analytical question that includes injection phrases
...
→ WHEN redaction or injection controls cannot initialize
...
→ THEN the system SHALL discard pending content and return a fixed localized failure message

### Requirement: Privacy verification corpus
The project SHALL include deterministic tests for every supported PII and secret category in answers, citations, dialog summaries, reports, and split streaming chunks. Verification MUST assert zero raw matches in captured outputs and telemetry traces.

#### Scenario: Run the privacy test suite
→ WHEN privacy tests execute against representative Chinese and English values
→ THEN all supported values SHALL be redacted and no raw sensitive value SHALL appear in captured outputs or logs